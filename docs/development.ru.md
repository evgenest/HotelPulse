...
- Один из локальных Kubernetes-вариантов:
  - `kind`: `winget install Kubernetes.kind` (Windows) или `brew install kind` (Mac)
  - `minikube`: `winget install Kubernetes.minikube` (Windows) или `brew install minikube` (Mac)
...
> Если Pod'ы не стартуют и показывают `ImagePullBackOff`, образы, скорее всего, были собраны вне окружения `minikube`. Повтори `eval $(minikube docker-env)` и пересобери образы.
